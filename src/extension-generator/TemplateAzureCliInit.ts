...
export function GenerateAzureCliInit(model: CodeModelAz) : string[] {
    ...
    output.push("        from azext_" + model.Extension_NameUnderscored + "._client_factory import cf_" + model.Extension_NameUnderscored + "");
    ...
    output.push("        from azext_" + model.Extension_NameUnderscored + ".commands import load_command_table");
    output.push("        load_command_table(self, args)");
    output.push("        return self.command_table");
    output.push("");
    output.push("    def load_arguments(self, command):");
    output.push("        from azext_" + model.Extension_NameUnderscored + "._params import load_arguments");
