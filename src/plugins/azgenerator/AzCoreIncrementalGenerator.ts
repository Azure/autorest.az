    ...
    public async generateAll(): Promise<void> {
        ...
        for (var ci = 0; ci < config.length; ci++) {
            ...
            if(keyName == "name" || config.length == 0){
                ...
                this.files[path.join(PathConstants.testFolder, PathConstants.latestFolder, PathConstants.incTestScenarioFile(keyName))] = GenerateAzureCliTestScenario(this.model,value);
            }
        }
        if (NeedPreparer()) {
