                ...
                if (isWhereCommandDirective(directive)) {
                    ...
                    for (const operationGroup of values(this.codeModel.operationGroups)) {
                        ...
                        if (!isNullOrUndefined(operationGroup.language['az']['command']) && operationGroup.language['az'].command.match(groupRegex)) {
                            const index = this.allCommandGroups.indexOf(operationGroup.language['az'].command);
                            if (index > -1) {
                            ...
                            operationGroup.language['az'].description = groupDescriptionReplacer || operationGroup.language['az'].description;
                            groupChanged = true;
                        }
                        let opIndex = -1;
                        for (const operation of values(operationGroup.operations)) {
                            ...
                            if (operation.language['az'].command !== undefined && operation.language['az'].command.match(commandRegex)) {
                                if (!isNullOrUndefined(commandRegex) && !isNullOrUndefined(commandReplacer)) {
                                    ...
                                    const newCommand = operation.language['az'].command.replace(commandRegex, commandReplacer);
                                    ...
                                    const newAzName = newCommandArr.length > 2 ? newCommandArr.slice(2, newCommandArr.length).join(' ') : newCommandArr.last;
                                    this.session.message({ Channel: Channel.Warning, Text: ' newAzName:' + newAzName });
                                    newCommandArr.pop();
                                    const newGroup = newCommandArr.length >= 2 ? newCommandArr[0] + ' ' + newCommandArr[1] : newCommandArr.join(' ');

                                    oldCommandArr.pop();
                                    const oldGroup = oldCommandArr.length >= 2 ? oldCommandArr[0] + ' ' + oldCommandArr[1] : oldCommandArr.join(' ');
                                    if (oldGroup !== newGroup) {
                                        ...
                                        if (operationGroup.operations.length === 1) {
                                            ...
                                            const oldIndex = this.allCommandGroups.indexOf(oldGroup);
                                            if (oldIndex > -1) {
                                            ...
                                            const newIndex = this.allCommandGroups.indexOf(newGroup);
                                            if (newIndex > -1) {
                                                operation.language['az'].command = newCommand;
                                                operation.language['az'].description = commandDescriptionReplacer || operation.language['az'].description;
                            ...
                            for (const parameter of values(operation.parameters)) {
                                if (parameter.language['az'].name !== undefined && parameter.language['az'].name.match(parameterRegex)) {
                                    ...
                                    parameter.language['az'].description = paramDescriptionReplacer || parameter.language['az'].description;
                                }
                            }
